...
tempWordlist = []   
def precomputeProbs(wordlist):
     for (x, c) in wordlist:
        prob =probabilityPre(x, wordlist, c)     
        tempWordlist.append(x, c, prob )


def convertToDict(wordlist):
    wordPreProbDict = {}
    for (i, (w, cl)) in enumerate(words):
        print("Creating dict: " + str(i) + " of " + str(len(words)))
        wordPreProbDict.update({(w, cl): probabilityPre(w, words, cl)})
    return wordPreProbDict
...
wordProbabilities = convertToDict(words)
